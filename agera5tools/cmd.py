...
def cmd_extract_point(agera5_path, longitude, latitude, startdate, enddate, output=None, tocelsius=False):
    ...
    :param agera5_path: path to the AgERA5 dataset\n
    :param longitude: the longitude for which to extract [dd, -180:180]\b
    :param latitude: the latitude for which to extract [dd, -90:90]\b
    :param startdate: the start date (yyyy-mm-dd, >=1979-01-01)\b
    :param enddate: the last date (yyyy-mm-dd, <= 1 week ago)\b
    """
...
@click.option("--add_gridid", help="Adds a grid ID instead of latitude/longitude columns.",
              is_flag=True)
def cmd_dump(agera5_path, day, output=None, tocelsius=False, add_gridid=False):
    day = check_date(day)
    output = Path(output)
    df = dump(agera5_path, day, tocelsius, add_gridid)
    if output is not None:
        output = Path(output)
    write_dataframe(df, output)
...
@click.option('--box', nargs=4, type=float,
              help=("Bounding box: <lon_min> <lon_max> <lat_min< <lat max>"))
def cmd_clip(agera5_path, day, output_dir=None, box=None, base_fname="agera5_clipped"):
    ...
    bbox = BoundingBox() if box is None else BoundingBox(*box)
